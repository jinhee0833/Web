	...
	<tr>
		...
		<td style='font-size:1px; width:2px; border:1px solid #000;'>
			<table width="750" border="0" cellspacing=0 cellpadding=0>
				...
				%>
					<td><a href="detail.do?idx=<%= bean.getBrd_idx() %>&pageNUM=<%= pageNUM %>&flag=true&action=detail">
...
<%
	...
	if(startPage-pageSize>0){
		out.println("<a href='myList.do?pageNUM="+(startPage-1)+"'>[이전]</a>");
	}
	
	//◀
	if(pageNUM-1 > 0){
		out.println("<a href='myList.do?pageNUM="+(pageNUM-1)+"'><font size=2 >◀</font></a>");
	}
	...
	for(int i=startPage ; i<startPage+pageSize ; i++){
		if(i==pageNUM){
			...
			out.println("<a href='myList.do?pageNUM="+i+"'> ["+i+"]</a>");
		}
	...
	if(startPage+pageSize <= pageCount){
		out.println("<a href='myList.do?pageNUM="+(startPage+pageSize)+"'>[다음]</a>");
	}
%>
								</td>
							</tr>
