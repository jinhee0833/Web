...
public class GDBSQL_re {
	...
	public List<DBbean_re> boardSelect(String pnum){
		...
		try{
			...
			System.out.println("guest/common/GDBSQL.java total= " + total()+", start="+start+", end="+end);
						
			sql="select * from (select rownum rn, num, title, contents, writer, wdate, count, reco, pwd from reviewboard order by wdate desc) where rn between "+end+" and "+start;
		...
		}catch(Exception ex){ System.out.println("guest/common/GDBSQL_re.java  List<DBbean_re> boardSelect(String pnum)이용후기 List 오류  : "+ex.toString()); }
		return list;
	}//select() END
