...
public class DBSQL {
	...
	public int dbInsert(DBbean bean){//InsertController.java연결
		...
		try{
				...
				PST.setString(4, bean.getGender());// (4, data4)
				...
				PST.setString(9, bean.getMem());// (3, data3)
	...
	public ArrayList<DBbean> getAddrs(String search) throws SQLException{
		ArrayList<DBbean> list = new ArrayList<DBbean>();
		try{
		String sql = "select zip_code, addrs from post where addrs like '%"+search+"%'";
		//String sql2 = new String(sql.getBytes("8859_1"), "UTF-8");
		ST=CN.createStatement();
		RS=ST.executeQuery(sql);
		while(RS.next()){
			DBbean bean = new DBbean();
			bean.setZip_code(RS.getInt("zip_code"));
			bean.setAddrs(RS.getString("addrs"));
			list.add(bean);
		}//while end
	...
	public int checkID(String id){
		int ok=0;
		try{
			sql="select count(*) as cnt from login where id='" +id+"'";
