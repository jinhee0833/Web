...
%>
		...
		<div id="event_contents">
			...
			<form name=delete method='post' action=eventDelete.do>
			  <input type=hidden name=num value=<%= num %>>
			  <input type=submit value=이벤트삭제 class="bt">
			</form>
		</div>
...
%>	
	<!-- 이벤트 리스트 -->
		<table id="admin_tableF">
			<tr><td colspan="4" id="admin_total" align="left">전체글 (<%=total %>)</td></tr>
			<tr class="admin_tableB">
				<td width=8%>NO</td>
		...
		%>
			<tr onMouseOver="style.backgroundColor='#ebe7e7'" onMouseOut="style.backgroundColor=''" class="td" >
				...
				<a href="event.do?num=<%= bean.getNum() %>">
	...
	<!-- 페이징 -->
		<div id="admin_page">
			...
			if(startPage > pageSize){ 
				out.println("<a href='event.do?pageType="+pageType+"&pageNUM="+(startPage-pageSize)+"'>◀ 이전</a>");
		 	}
			for(int i=startPage; i<(startPage+pageSize); i++){
				if(i==pageNUM){ out.println("<font color=#6e6464>["+i+"]</font>"); }
				else{ 
					out.println( "<a href='event.do?pageType="+pageType+"&pageNUM="+i+"'>"+i+"</a>");
				}
				if(i>=pageCount) break;
			}		
			if(pageSize <= pageCount-startPage){
				out.println("<a href='event.do?pageType="+pageType+"&pageNUM="+(startPage+pageSize)+"'>다음 ▶</a>");  
			}			
			%>
		</div>
	</div>	
	<!-- 버튼 -->
	<form name=save method='post' action=./index.jsp?page=admin/m_admin&list=eventInsert>
	  <input type=submit value=등록 class="bt">
	</form>
	<a href="event.do?&pageType=now"><input type=submit value=진행이벤트 class="bt"></a>
	<a href="event.do?&pageType=past"><input type=submit value=종료이벤트 class="bt"></a>
</div>	
</body>
</html>